...
function getCSRFSecret(): string {
  ...
  if (!secret) {
    // In production, log warning but use a fallback to prevent 500 errors
    if (process.env.NODE_ENV === 'production') {
      console.warn('[CSRF] WARNING: CSRF_SECRET not configured. Using fallback (insecure). Set CSRF_SECRET in Vercel environment variables.');
      // Return a consistent but insecure fallback for production
  ...
  if (secret.length < 64) {
    if (process.env.NODE_ENV === 'production') {
      console.warn('[CSRF] WARNING: CSRF_SECRET is too short. Using it anyway to prevent 500 errors.');
    } else {
      throw new Error(
